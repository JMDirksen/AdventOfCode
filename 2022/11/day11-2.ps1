...
for ($round = 1; $round -le 10000; $round++) {
    ...
    foreach ($monkey in $monkeys) {
        "Monkey $($monkey.id) has $($monkey.items.Count) items and inspected $($monkey.inspected)"
        # Items
        while ($monkey.items.Count) {
            ...
            $monkey.inspected++
            # Operation
            [bigint]$item = iex $monkey.operation.Replace("old", $item)
            # Relief
        ...
        "Monkey $($monkey.id) inspected $($monkey.inspected)"
    }
...
"inspects $($inspects | ConvertTo-Json -Compress)"
